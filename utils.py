...
from info import AUTH_CHANNEL, IS_SHORT_DROPLINK_URL, LONG_IMDB_DESCRIPTION, MAX_LIST_ELM, DROPLINK_API, LONG_DROPLINK_URL, SHORTENER_WEBSITE_URL
...
async def droplink_url(link, x=""):
    ...
    if not IS_SHORT_DROPLINK_URL:
        return link

    if LONG_DROPLINK_URL == "True" or LONG_DROPLINK_URL is True:
        text = f'https://{SHORTENER_WEBSITE_URL}/st?api={DROPLINK_API}&url={url}'
        return text
    else:
        https = link.split(":")[0]
        if "http" == https:
        ...
        url = f'https://{SHORTENER_WEBSITE_URL}/api'
        ...
        try:
            ...
            logger.error(e)
            links = f'https://{SHORTENER_WEBSITE_URL}/st?api={DROPLINK_API}&url={link}'
